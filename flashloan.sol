...
contract SampleLoan is FlashLoanReceiverBase {
    address private multiDaiKovan = 0xFf795577d9AC8bD7D90Ee22b6C1703490b6512FD;
    address private usdc = 0xe22da380ee6B445bb8273C81944ADEB6E8450422;
    ...
    function swapTokens(
        ...
        IERC20(path[0]).safeApprove(router, inputAmount);
    ...
    function addLiquidity(
        ...
        IERC20(token1).transferFrom(msg.sender, address(this), iAmount);
        IERC20(token2).transferFrom(msg.sender, address(this), jAmount);
        IERC20(token1).safeApprove(router, iAmount);
        IERC20(token2).safeApprove(router, jAmount);
    ...
    function convertDaiToUsdc(uint256 daiAmount) public {
        uint256 deadline = block.timestamp + 15; // using 'now' for convenience, for mainnet pass deadline from frontend!

        address[] memory path = new address[](2);
        path[0] = usdc;
        path[1] = multiDaiKovan;
        IERC20(usdc).transferFrom(msg.sender, address(this), daiAmount);
        IERC20(path[0]).safeApprove(UNISWAP_ROUTER_ADDRESS, daiAmount);
    ...
    {
        return uniswapRouter.getAmountsIn(daiAmount, getPathForDAItoEth());
    }
